...
public class FirebaseWrite {
    ...
    public static void updateUserProfile(String name, String phoneNumber) {
        ...
        if (AppData.userProfileType != null) {
            AppData.userProfileType.setName(name);
            AppData.userProfileType.setPhoneNumber(phoneNumber);
        } else {
            AppData.userProfileType = new UserProfileType(AppData.firebaseAuth.getCurrentUser().getEmail(), name, phoneNumber);
        }

        ref.setValue(AppData.userProfileType);
    }
